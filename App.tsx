...
const PatientView = ({ socket }: { socket: any, key?: string }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const systemInstruction = `You are Dr Dost, an expert AI Health & Ayurvedic consultant for the SwiftRescue emergency and medical platform. Your goal is to provide safe, traditional Ayurvedic home remedies and immediate health guidance based on the symptoms or images described/shown by the user.
...
DOCTOR CONSULTATION & EMERGENCY INSTRUCTION:
- If the user mentions "doctor", "clinic", "hospital", "appointment", "consultation", "specialist", or "nearby help", recommend consulting qualified physicians and highlight nearby medical centers.
- If the user describes severe emergency symptoms (e.g., severe chest pain, difficulty breathing, profuse bleeding, sudden numbness/paralysis, unconsciousness), immediately urge calling emergency ambulance dispatch (using the SOS button in this app) and seeking immediate emergency medical care.

AYURVEDIC & HEALTH ANALYSIS:
- Analyze symptoms and skin/body conditions with compassionate, actionable wellness guidance and Dosha balance principles.
...
- Keep responses clean, concise, warm, structured with bullet points.`;
      ...
      if (isGroq && activeApiKey) {
        ...
        if (currentImage) {
          messages.push({
            role: "user",
            content: [
              { type: "text", text: userMessage || "Analyze this medical condition/image and provide Ayurvedic advice." },
      ...
      // Voice Output: Browser SpeechSynthesis + Gemini TTS fallback
      if (botResponse && typeof window !== 'undefined') {
        try {
      ...
      setChatMessages(prev => [...prev, { 
        role: 'bot', 
        text: `Error connecting to AI: ${error?.message || "Please check your network connection and API key."}` 
      }]);
    } finally {
      setIsTyping(false);
    }
